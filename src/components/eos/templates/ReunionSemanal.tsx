            ...
            <div key={r.id} className="rounded-lg border border-border bg-card overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <Badge variant={r.estado === "preparada" ? "secondary" : "default"} className={badge.cls}>
                    {badge.label}
                  </Badge>
                ...
                <div className="border-t border-border">
                  ...
                  <div className="px-5 py-5 space-y-6 print:hidden">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        {(["preparada", "en_curso", "completada"] as EstadoReunion[]).map((s) => (
                          <button
                            key={s}
                            onClick={() => update(r.id, { estado: s })}
                            className={`rounded border px-2 py-0.5 text-xs transition-colors ${
                              r.estado === s
                                ? s === "preparada" ? "border-border bg-muted font-medium text-foreground"
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      ...
                      <div><Label className="text-xs">Participantes</Label><Input className="mt-1" value={r.participantes} onChange={(e) => update(r.id, { participantes: e.target.value })} placeholder="Nombres o nº" /></div>
                    ...
                    <div className="border-t border-border pt-5">
                      ...
                      <Textarea className="mt-2 min-h-[160px] bg-background" value={r.acta || ""}
                        onChange={(e) => update(r.id, { acta: e.target.value })} placeholder="Escribe o dicta el acta…" />
                    </div>
                  ...
                  {/* Print content (this specific meeting) */}
                  <PrintLayout
                    title="Reunión Semanal (Nivel 10)"
                    clienteNombre={clienteNombre}
                    date={formatFecha(r.fecha)}
                  >
                    <PrintSection label="Reunión">
                      <PrintRow label="Participantes" value={r.participantes} />
                      <PrintRow label="Duración" value={r.duracion} />
                      <PrintRow label="Estado" value={BADGE[r.estado]?.label} />
                      {r.valoracion !== null && <PrintRow label="Valoración" value={`${r.valoracion}/10`} />}
                    </PrintSection>

                    <PrintSection label="Agenda">
                      {AGENDA.map(({ key, label }) =>
                        (r[key] as string) ? <PrintField key={key} label={label} value={r[key] as string} /> : null
                      )}
                    </PrintSection>

                    {r.acta && (
                      <PrintSection label="Acta">
                        <PrintField label="" value={r.acta} />
                      </PrintSection>
                    )}
                  </PrintLayout>
                </div>
