...
function SortableDept({ dept, onRemoveDept, onUpdateNombre, onAddItem, onRemoveItem, onUpdateItem }: {
  ...
  return (
    <div ref={setNodeRef} style={style} className="rounded border border-amber-200 bg-white/80 p-3 dark:border-amber-800 dark:bg-amber-950/40">
...
export function Asuntos({ clienteId, clienteNombre }: NoClientProps) {
  const { data, setData, saveNow, saving, loading } = usePlantilla<Data>(clienteId, "asuntos", DEFAULT);
  ...
  useEffect(() => {
    ...
    if (needsNorm(data.vision) || needsNorm(data.semanales) || (data.departamentos || []).some((d) => needsNorm(d.items))) {
      setData({ vision: toItems(data.vision), semanales: toItems(data.semanales), departamentos: (data.departamentos || []).map((d) => ({ ...d, items: toItems(d.items) })) });
    }
  ...
  const updateVision = (id: string, patch: Partial<Item>) => setData({ ...data, vision: vision.map((i) => i.id === id ? { ...i, ...patch } : i) });
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const getList = (colId: string): Item[] => { if (colId === "vision") return vision; if (colId === "semanales") return semanales; return departamentos.find((d) => d.id === colId)?.items || []; };
  ...
  if (viewMode) {
    return (
      <DocumentViewer title="Asuntos" clienteNombre={clienteNombre}
  ...
  return (
    <div className="max-w-5xl">
      ...
      <FieldHint>Los Asuntos son problemas, obstáculos o temas pendientes a resolver. Clic en un asunto para editarlo.</FieldHint>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50/70 p-4 dark:border-amber-800 dark:bg-amber-950/20">
            <div className="mb-1 flex items-center justify-between">
              <h2 className="text-sm font-display font-semibold">Asuntos por Área</h2>
              <button onClick={addDept} className="text-primary hover:text-primary/80 transition-colors print:hidden" title="Añadir área"><FolderPlus className="h-4 w-4" /></button>
            </div>
            <FieldHint>Asuntos específicos por área o departamento</FieldHint>
            <div className="mt-3 space-y-3">
              {departamentos.length === 0 && <p className="text-xs italic text-muted-foreground">Pulsa + para crear un área.</p>}
