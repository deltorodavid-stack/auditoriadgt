...
function generateMd(r: Reunion, clienteNombre: string): string {
  ...
  for (const { key, label } of AGENDA) { if (r[key]) lines.push(`\n**${label}**\n${r[key] as string}`); }
  ...
  if (viewingId) {
    ...
    if (r) return (
      <DocumentViewer title="Reunión Trimestral" clienteNombre={clienteNombre}
        ...
        mdContent={generateMd(r, clienteNombre)} mdFilename={makeMdFilename("reunion-trimestral", clienteNombre, r.fecha)}>
        ...
        <DocSection label="Agenda">
          {AGENDA.map(({ key, label }) => (r[key] as string) ? <DocField key={key} label={label} value={r[key] as string} /> : null)}
        </DocSection>
