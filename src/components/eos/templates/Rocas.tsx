  ...
  return (
    <div ref={setNodeRef} style={style}
      ...
      {editing ? (
        ...
        >
          {item.texto || <span className="italic text-muted-foreground/60">Roca sin texto</span>}
        </span>
      )}
...
export function Rocas({ clienteId, clienteNombre }: NoClientProps) {
  ...
  const updateEmpresa = (id: string, patch: Partial<Item>) =>
    setData({ ...data, empresa: empresa.map((i) => i.id === id ? { ...i, ...patch } : i) });
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const getList = (colId: string): Item[] => {
      if (colId === "empresa") return empresa;
  ...
  return (
    <div className="max-w-5xl">
      ...
      <FieldHint>
        ...
        Haz clic en cualquier roca para editarla. Arrastra el icono <GripVertical className="inline h-3 w-3" /> para reordenar.
      </FieldHint>
