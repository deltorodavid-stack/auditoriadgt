...
export function AuditSidebar({ mobileOpen = false, onClose }: AuditSidebarProps) {
  ...
  const handleSelectBlock = (n: number) => {
    setCurrentBlock(n);
    onClose?.();
  };
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 z-40 flex h-screen w-64 flex-col border-r border-border bg-sidebar transition-transform duration-200 md:translate-x-0 ${
          ...
          <ul className="space-y-1">
            {AUDIT_BLOCKS.map((block) => {
              ...
              return (
                <li key={block.number}>
                  <button
                    onClick={() => handleSelectBlock(block.number)}
